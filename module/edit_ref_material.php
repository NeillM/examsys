...
<div class="head_title" style="font-size:90%">
  ...
  <div class="breadcrumb"><a href="../index.php"><?php echo $string['home'] ?></a><a href="details.php?module=<?php echo $_GET['module'] ?>"><?php echo module_utils::get_moduleid_from_id($_GET['module'], $mysqli); ?></a><a href="list_ref_material.php?module=<?php echo $_GET['module']; ?>"><?php echo $string['referencematerial']; ?></a></div>
...
foreach ($module_array as $modID => $module) {
    ...
    if ($match == true) {
        if ($userObject->is_staff_user_on_module($modID) or $userObject->has_role('SysAdmin')) {
            echo "<div class=\"r2\" id=\"divmod$module_no\"><input type=\"checkbox\" name=\"mod$module_no\" id=\"mod$module_no\" value=\"$modID\" checked><label for=\"mod$module_no\">" . $module['id'] . ': ' . mb_substr((string) $module['fullname'], 0, 60) . "</label></div>\n";
        } else {
            echo "<div class=\"r2\" id=\"divmod$module_no\"><input type=\"checkbox\" name=\"dummymod$module_no\" value=\"" . $module['id'] . "\" checked disabled><input type=\"checkbox\" name=\"mod$module_no\" id=\"mod$module_no\" style=\"display:none\" value=\"$modID\" checked>" . $module['id'] . ': ' . mb_substr((string) $module['fullname'], 0, 60) . "</div>\n";
        }
    } else {
        echo "<div class=\"r1\" id=\"divmod$module_no\"><input type=\"checkbox\" name=\"mod$module_no\" id=\"mod$module_no\" value=\"$modID\"><label for=\"mod$module_no\">" . $module['id'] . ': ' . mb_substr((string) $module['fullname'], 0, 60) . "</label></div>\n";
    }
    $module_no++;
