<?php           //cz
$string['machine'] = 'Počítač';
$string['machines'] = 'Počítače';
$string['createnewlab'] = 'Nová učebna';
$string['vieweditdetails'] = 'Zobrazit/Upravit detaily';
$string['deletelab'] = 'Odstranit učebnu';
$string['lowbandwidth'] = 'Pomalé připojení';
$string['name'] = 'Název';
$string['campus'] = 'Kampus';
$string['building'] = 'Budova';
$string['roomnumber'] = 'Číslo místnosti';
$string['bandwidth'] = 'Připojení';
$string['low'] = 'Pomalé';
$string['high'] = 'Rychlé';
$string['timetabling'] = 'Rozvrh';
$string['itsupport'] = 'IT Podpora';
$string['plagarism'] = 'Podvádění';
$string['ipaddresses'] = 'IP Adresy';
$string['save'] = 'Uložit';
$string['badaddressesmsg'] = 'Následující IP adresa byla chybná a nebyla zařazena do učebny: %s. Ostatní adresy byly uloženy.';
$string['backtolabs'] = 'Zpět na seznam učeben';
$string['noipaddresses'] = 'Učebnu nelze vytvořit, nejsou uvedeny žádné IP adresy.';
?>