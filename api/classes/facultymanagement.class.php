...
class facultymanagement extends \api\abstractmanagement
{
    ...
    public function create($params, $userid)
    {
        ...
        $strings = $langpack->get_strings($this->langcomponent, ['faculty_not_created', 'faculty_already_exists', 'faculty_name_not_supplied']);

        // Name must be supplied.
        if (!isset($params['name']) or $params['name'] === '') {
            $data = ['statuscode' => $this->statuscodes['FACUTLY_NAME_NOT_SUPPLIED'], 'status' => $strings['faculty_name_not_supplied'], 'id' => null, 'externalid' => null];
        } else {
            // Create faculty.
            $facultyid = false;
            ...
            if (!$facultyid) {
                ...
                if ($id) {
                    ...
                    $data = ['statuscode' => $this->statuscodes['FACUTLY_NOT_CREATED'], 'status' => $strings['faculty_not_created'], 'id' => null, 'externalid' => null];
                }
            } else {
                $details = \FacultyUtils::get_faculty_details_by_id($facultyid, $this->db);
                $externalid = $details['externalid'];
                $data = ['statuscode' => $this->statuscodes['FACUTLY_ALREADY_EXISTS'], 'status' => $strings['faculty_already_exists'], 'id' => $facultyid, 'externalid' => $externalid];
            }
        }
    ...
    {
        $langpack = new \langpack();
        $strings = $langpack->get_strings($this->langcomponent, ['faculty_not_updated', 'faculty_does_not_exist'
        ...
        if ($facultyid) {
            if ($change) {
                ...
                if ($update) {
                    ...
                    $data = ['statuscode' => $this->statuscodes['FACUTLY_NOT_UPDATED'], 'status' => $strings['faculty_not_updated'], 'id' => null, 'externalid' => null];
                }
            } else {
                $data = ['statuscode' => $this->statuscodes['FACUTLY_NOTHING_TO_UPDATE'], 'status' => $strings['faculty_nothing_to_update'], 'id' => null, 'externalid' => null];
            }
        } else {
            $data = ['statuscode' => $this->statuscodes['FACUTLY_DOES_NOT_EXIST'], 'status' => $strings['faculty_does_not_exist'], 'id' => null, 'externalid' => null];
        }
    ...
    {
        $langpack = new \langpack();
        $strings = $langpack->get_strings($this->langcomponent, ['faculty_not_deleted_inuse', 'faculty_not_deleted'
        ...
        if ($facultyid) {
            ...
            if (isset($schools) and $schools > 0) {
                $data = ['statuscode' => $this->statuscodes['FACUTLY_NOT_DELETED_INUSE'], 'status' => $strings['faculty_not_deleted_inuse'], 'id' => null, 'externalid' => null];
            } else {
                $deleted = \FacultyUtils::delete_faculty($params['id'], $this->db);
                if ($deleted) {
                    $data = ['statuscode' => $this->statuscodes['OK'], 'status' => 'OK', 'id' => $params['id'], 'externalid' => $details['externalid']];
                } else {
                    $data = ['statuscode' => $this->statuscodes['FACUTLY_NOT_DELETED'], 'status' => $strings['faculty_not_deleted'], 'id' => null, 'externalid' => null];
                }
            }
        } else {
             $data = ['statuscode' => $this->statuscodes['FACUTLY_DOES_NOT_EXIST'], 'status' => $strings['faculty_does_not_exist'], 'id' => null, 'externalid' => null];
