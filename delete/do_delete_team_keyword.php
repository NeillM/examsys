...
$keyword_names = [];
$result = $mysqli->prepare('SELECT keyword FROM keywords_user WHERE id IN (' . mb_substr((string) $keywordIDs, 1) . ')');
$result->execute();
...
$result->close();
if (count($keyword_names) < mb_substr_count((string) $keywordIDs, ',')) {
...
$keyword_list = explode(',', mb_substr((string) $keywordIDs, 1));
