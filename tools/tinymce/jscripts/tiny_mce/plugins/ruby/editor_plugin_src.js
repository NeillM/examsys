 ...
 *
 * Copyright 2017, Nottignham University
...
(function() {
	...
	tinymce.create('tinymce.plugins.RubyPlugin', {
		...
		init : function(ed, url) {
			// Register the command so that it can be invoked by using tinyMCE.activeEditor.execCommand('mceRuby');
			ed.addCommand('mceRuby', function() {
				ed.windowManager.open({
					file : url + '/dialog.htm',
					width : 320 + parseInt(ed.getLang('ruby.delta_width', 0)),
					height : 120 + parseInt(ed.getLang('ruby.delta_height', 0)),
					inline : 1
				}, {
			...
			// Register ruby button
			ed.addButton('ruby', {
				title : 'ruby.desc',
				cmd : 'mceRuby',
				image : url + '/img/ruby.png'
			});
