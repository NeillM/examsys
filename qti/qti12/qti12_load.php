...
class IE_qti12_Load extends IE_Main
{
    ...
    public function Load($params)
    {
        ...
        $xmlStr = str_replace(['‘','’'], ["'","'"], $xmlStr, $count);
        ...
        $pos = mb_strpos($xmlStr, 'wct_questiontype');
    ...
    public function LoadItem($item)
    {
        ...
        if ($type == 'blank') {
            ...
            $question = $this->LoadTrueFalse($q_imp);
    ...
    public function DetermineQType(&$question)
    {
        ...
        if ($question->counts['lid'] == 1) {
            ...
            if ($question->qmd_itemtype == 'Multiple Choice') {
                return 'mcq';
            }
            if ($question->qmd_itemtype == 'True False') {
                return 'truefalse';
            }
            if ($question->wct_questiontype == 'WCT_TrueFalse') {
                return 'truefalse';
            }
        ...
        if ($question->counts['lid'] > 1) {
            ...
            if ($question->cardinality == 'Single') {
                ...
                if ($question->qmd_itemtype == 'Extended Matching') {
                    return 'extmatch';
                }
    ...
    public function ReplaceTitle(&$title, $scn)
    {
        ...
        foreach ($bits as $bit) {
            ...
            if (mb_substr($bit, mb_strlen($bit) - 1, 1) == '>') {
                continue;
            }

            if (mb_strpos(' ' . $bit, '>') > 0) {
                $bit = mb_substr($bit, mb_strpos($bit, '>') + 1);
            }
            ...
            if (mb_stripos(' ' . $title, $bit) > 0) {
                $to_remove[] = $bit;
                $removed_count += mb_strlen($bit);
                $newtitle .= $bit . ' ';
                if ($removed_count > 10) {
                    $title = trim($newtitle);
                            ...
                            if ($subname == 'material') {
                                ...
                                foreach ($source->respconditions as $respcondition) {
                                    ...
                                    foreach ($respcondition->conditions as $condition) {
                                        ...
                                        if ($condition->index && $condition->index != $index) {
                                            continue;
                                        }
                                        if ($condition->respident != $response->id) {
                                            continue;
                                        }
    ...
    public function LoadCalculation(&$source)
    {
        ...
        foreach ($respconds as $respcond) {
            foreach ($respcond->conditions as $cond) {
                if ($cond->type == 'varequal') {
                    ...
                    break;
                } elseif ($cond->type == 'vargte' || $cond->type == 'vargt') {
                    $morethan = $cond->value;
                } elseif ($cond->type == 'varlte' || $cond->type == 'varlt') {
                    $lessthan = $cond->value;
                }
            }
        ...
        if (array_key_exists('SETTINGS', $source->params) && array_key_exists('QUESTION', $source->params)) {
            if (array_key_exists('SETTINGS', $source->params)) {
                $dest->settings = $source->params['SETTINGS'];
            }
        }
        ...
        foreach ($conds as $condition) {
            if (count($condition->conditions) == 0) {
                ...
                if (count($condition->conditions) > 1) {
                    $this->AddWarning($string['multiplepos'], $source->load_id);
                }
    ...
    public function LoadLabelling(&$source)
    {
        ...
        foreach ($source->responses as $response) {
            foreach ($response->labels as $label) {
                ...
                foreach ($source->respconditions as $respcondition) {
                    foreach ($respcondition->conditions as $condition) {
                        if ($condition->respident != $mylabel->id) {
                            continue;
                        }
                        if ($condition->not) {
                            continue;
    ...
    public function LoadTextbox(&$source)
    {
        ...
        if (count($conditions) > 0) {
            $this->AddWarning($string['importingtext'], $source->load_id);
        }
    ...
    public function GetResponseLabelList(&$question, $clean = true, &$lablk = [], &$lablkd = [])
    {
        $resplist = [];
        $numbb = 1;
        $loop = 0;
    ...
    public function getMarksFromRespConditions(&$data)
    {
        ...
        foreach ($data->respconditions as & $respconditions) {
            if (isset($respconditions->conditions[0]) and ($respconditions->conditions[0]->type == 'vargte' or $respconditions->conditions[0]->type == 'varlte')) {
                if ($respconditions->mark > $part) {
                    $part = $respconditions->mark;
                }
            } else {
                if ($respconditions->mark > $max) {
                    $max = $respconditions->mark;
                } elseif ($respconditions->mark < $min) {
                    $min = $respconditions->mark;
                }
            }
        }
    ...
    public function GetResponses(&$data, $type = '', $render = '')
    {
        ...
        foreach ($data->responses as & $response) {
            ...
            if ($render && $response->render != $render) {
                continue;
            }
    ...
    public function GetFeedbacks(&$source, $respident, $value = '', $match = 1)
    {
        ...
        foreach ($source->respconditions as & $respcondition) {
            if (count($respcondition->conditions) > 0 && $respcondition->conditions[0]->respident != $respident) {
                continue;
            }
