...
{	
	...
	function MakeQuestionHeader(&$question)	{
		...
		if ($question->notes)
			$output .= "<p>" . htmlentities($question->notes) . "</p>\n";
		
		if ($question->scenario)
			$output .= MakeNiceXHTML($question->scenario) . "\n";
		
		if ($question->leadin)
			$output .= MakeNiceXHTML($question->leadin) . "\n";
	...
	function SaveBlank(&$question) {
		...
		if (strtolower($question->displaymode) == "dropdown")
			include "qti20/tmpl/blank-dropdown.php";
		else 
			include "qti20/tmpl/blank-textentry.php";
	...
	function SaveMrq(&$question) {
		...
		if (strtolower($question->score_method) == "allitemscorrect")
			include "qti20/tmpl/mrq-allitemscorrect.php";
	...
	function GetTDSet($type,$correct,&$question = '')	{
		...
		{
			for ($i = $question->startyear ; $i <= $question->endyear ;  $i++)
				$res->values[] = sprintf("%02d",$i);
		} else if ($type == "hh")
		{
			for ($i = 0 ; $i < 24 ; $i++)
