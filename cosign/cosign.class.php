...
class cosign
{
    ...
    public function cosign_debug($str)
    {
        ...
        if (empty($this->cosign_cfg['CosignFilterDebug'])) {
            return;
        }
    ...
    public function cosign_auth($cfg = [], $obstart = true)
    {
        ...
        $proto = intval($code[1]);
        if (
            $proto < 2 && // cosign protocol version
        ...
        if (count($code) > 6 && strncasecmp($code[6], '[COSIGNv', 8) == 0) { // v3 protocol
            ...
            while (!str_ends_with($code[$i], ']')) {
                ...
                if (strncasecmp($code[$i], 'FACTORS', 7) == 0) {
                    $sup_factors = true;
                } elseif (strncasecmp($code[$i], 'REKEY', 5) == 0) {
                    $sup_rekey = true;
                } else {
        ...
        if ($rekey_service) {
            ...
            ob_end_clean();
            setrawcookie($service_cookie, $service_cookie_val . '/' . time(), ['expires' => 0, 'path' => '/', 'domain' => '', 'secure' => str_starts_with($dest, 'https://') ? true : false, 'httponly' => true]);
    ...
    public function cosign_set_cookie_and_redirect($url = '')
    {
        if ($url) {
            ...
            if (
                !empty($this->cosign_cfg['CosignSiteEntry']) &&
            ...
            if ($this->cosign_cfg['CosignProtocolVersion'] <= 2) {
                ...
                setrawcookie($service_cookie, $service_cookie_val . '/' . time(), ['expires' => 0, 'path' => '/', 'domain' => '', 'secure' => str_starts_with($back, 'https://') ? true : false, 'httponly' => true]);
            } else {
    ...
    public function cosign_check_factors($fa, $suffix = false)
    {
        ...
        foreach ($req_fac as $rf) {
            ...
            if (
                $suffix && isset($this->cosign_cfg['CosignFactorSuffix']) &&
                in_array($rf . $this->cosign_cfg['CosignFactorSuffix'], $sc_fac)
            ) {
                if (empty($this->cosign_cfg['CosignFactorSuffixIgnore'])) {
                    continue;
                }
