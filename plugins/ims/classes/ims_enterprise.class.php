...
class ims_enterprise
{
    ...
    public function process()
    {
        ...
        if (empty($this->filename)) {
            ...
            } elseif (isset($this->prevtime) && ($filemtime <= $this->prevtime)) {
                $this->log_line('File modification time is not more recent than last update - skipping processing.');
            } elseif (isset($this->prevmd5) && ($md5 == $this->prevmd5)) {
        ...
        switch ($recstatus) {
            ...
            default:
                ...
                if (!$moduleid) {
                    ...
                    $moduleid = \module_utils::add_modules(
                        $modulecode,
                        $fullname,
                        $active,
                        $schoolid,
    ...
    protected function process_member($member, $modulecode)
    {
        ...
        } elseif ($status == self::ROLE_STATUS_ACTIVE) {
            $success = \UserUtils::add_staff_to_module_by_modulecode($userid, $modulecode, $this->db);
        } elseif ($status == self::ROLE_STATUS_INACTIVE && $role === 'Staff') {
