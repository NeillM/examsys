...
<tr>
<th colspan="3"><div class="breadcrumb"><a href="../staff/index.php"><?php echo $string['home']; ?></a>&nbsp;&nbsp;<img src="../artwork/breadcrumb_arrow.png" width="4" height="7" alt="-" />&nbsp;&nbsp;<a href="./index.php"><?php echo $string['administrativetools']; ?></a></div><div style="margin-left:10px; font-size:200%; font-weight:bold"><?php echo $string['summativescheduling']; ?></th>
...
<tr>
<th><div class="mid"><?php echo $string['title']; ?>&nbsp;</div></th>
...
<?php
  ...
  $results = $mysqli->prepare("SELECT property_id, paper_title, moduleID, period, barriers_needed, cohort_size, notes FROM (properties, scheduling) WHERE (start_date IS NULL OR end_date IS NULL) AND properties.property_id=scheduling.paperID");
  $results->execute();
  $results->store_result();
  $results->bind_result($property_id, $paper_title, $moduleID, $period, $barriers_needed, $cohort_size, $notes);
  while ($results->fetch()) {
    $cohort_size = str_replace('<', '&lt;', $cohort_size);
    $cohort_size = str_replace('>', '&gt;', $cohort_size);
    echo "<tr onclick=\"sel($property_id)\" onmouseover=\"lon($property_id)\" onmouseout=\"loff($property_id)\" ondblclick=\"viewDetails()\" id=\"$property_id\"><td>$paper_title</td><td>" . $string[$months[$period]] . "</td><td>$moduleID</td><td>$cohort_size</td><td>$notes</td></tr>";
  }
  $results->close();
?>
</table>
