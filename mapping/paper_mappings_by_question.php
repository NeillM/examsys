...
<?php
  ...
  <td class="taboff" onclick="window.location.href='paper_mappings_by_year.php?paperID=<?php echo $paperID; ?>&folder=<?php echo $_GET['folder']; ?>&module=<?php echo $_GET['module']; ?>'"><?php echo $string['longitudinal']; ?></td>
  </table>
    ...
    if ($temp_array[$x]['leadin'] != '') {
      ...
      if (count($objByModule) > 0) {
        if (isset($objByModule['none_of_the_above']['mapped']) and $objByModule['none_of_the_above']['mapped'] == 1) {
          echo "<ul class=\"$class\" style=\"list-style-type:none; margin-left:10px; padding:0px\">\n<li style=\"padding-left:10px; color:red; background-image:url(../artwork/small_yellow_warning_icon.gif); background-repeat:no-repeat\"><strong>" . $string['warning'] . ":</strong> " . $string['questiononnotmap'] . "</li></ul>\n";
        } else {
