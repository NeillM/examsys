...
class labs extends generator {
  ...
  protected function insert_campus($values) {
    $db = loader::get_database();
    $query = $db->prepare("INSERT INTO campus (name, isdefault) VALUES (?, ?)");
  ...
  protected function insert_exam_pc($values) {
    $db = loader::get_database();
    $query = $db->prepare("INSERT INTO client_identifiers (lab, address, hostname, low_bandwidth) VALUES (?, ?, ?, ?)");
