...
class modules extends generator {
     ...
     * options are ($active, $schoolID, $vle_api, $sms_api, $selfEnroll, $peer, $external, $stdset, $mapping, $neg_marking, $ebel_grid_template, $db, $sms_import = 0, $timed_exams = 0, $exam_q_feedback = 1, $add_team_members = 1, $map_level = 0, $academic_year_start = '07/01')
     * @throws Exception If passed parameter is invailid
     */
    public function create_module($parameters) {

        if (empty($parameters['moduleid']) or empty($parameters['fullname'])) {
            throw new data_error('Must pass parameter | moduleid | fullname |');
        } else {
    ...
    public function create_module_team($parameters) {
        include_once 'papers.php';
        $modulename = $parameters['modulename'];
        $username = $parameters['username'];
        $db = loader::get_database();
        $userid = papers::test_get_useridbyname($username, $db);
