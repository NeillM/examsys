...
<?php
  ...
  $result = $mysqli->prepare("SELECT DISTINCT(language) FROM staff_help WHERE language != 'en'");
  $result->execute();
  ...
  $result = $mysqli->prepare("SELECT articleid, title, body, type, roles FROM staff_help WHERE language = 'en' AND deleted IS NULL");
  $result->execute();
  ...
  foreach ($languages as $language) {
    echo "<h1>$language</h1>\n";
    echo "<ul>";
    $lang_pages = pages_in_lang($language, $mysqli);
    
    foreach ($en_pages as $pageID => $page_details) {
      if (!isset($lang_pages[$pageID])) {
        create_page($language, $pageID, $page_details, $mysqli);
...
function create_page($language, $articleid, $page_details, $db) {
  ...
  $result = $db->prepare("INSERT INTO staff_help VALUES (NULL, ?, ?, ?, ?, NULL, NULL, ?, NULL, ?, ?, '0000-00-00 00:00:00')");
	$result->bind_param('ssssssi', $title, $body, $body_plain, $type, $roles, $language, $articleid);
	$result->execute();
	$result->close();
}

function pages_in_lang($lang, $db) {
  $lang_pages = array();
  
  $result = $db->prepare("SELECT articleid FROM staff_help WHERE language = ? AND deleted IS NULL");
