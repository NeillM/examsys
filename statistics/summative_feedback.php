...
<head>
  ...
  <script language="JavaScript">
    function jumpTo() {
      document.location = 'summative_feedback.php?year=' + $('#year').val();
    }
  </script>
</head>
...
<tr>
<th colspan="2"><div style="margin-left:10px; font-size:200%"><strong><?php echo $string['summativeexamfeedback']; ?>:</strong> <?php echo $_GET['year']; ?>/<?php echo (substr($_GET['year'],2,2)+1); ?></th>
...
foreach ($master_array as $school => $data) {
	...
	if (count($moduleIDs) > 0) {
		...
		$result = $mysqli->prepare("SELECT DISTINCT properties.property_id, paper_title, paper_type FROM properties, properties_modules WHERE properties.property_id = properties_modules.property_id $date_range AND paper_type = '2' AND idMod IN (" . implode(',', $moduleIDs) . ") GROUP BY property_id");
		$result->execute();
		...
		$result = $mysqli->prepare("SELECT idfeedback_release, type FROM properties, properties_modules, feedback_release WHERE feedback_release.paper_id = properties.property_id AND properties.property_id = properties_modules.property_id $date_range AND paper_type = '2' AND idMod IN (" . implode(',', $moduleIDs) . ")");
		$result->execute();
