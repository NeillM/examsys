...
if ($updater_utils->check_version("6.1.0")) {
    if (!$updater_utils->has_updated('rogo1642_lticontextid')) {
        ...
        if ($lti_integration == 'UoN') {
            ...
            $select_sql = "SELECT c.c_internal_id, m.id FROM lti_context c, modules m WHERE ((LENGTH(c.c_internal_id) - LENGTH(REPLACE(c.c_internal_id, 'N', ''))) = 1
                AND (m.moduleid LIKE '%\_UNNC' and SUBSTRING_INDEX(c.c_internal_id, '-CN', 1) = SUBSTRING_INDEX (m.moduleid, '_', 1))
