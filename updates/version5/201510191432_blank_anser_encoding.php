 ...
 * @package update
 */
if (!$updater_utils->has_updated('blank_answer_encoding')) {
  // Find all the answers for fill in the blank questions.
  $select_sql = "SELECT id, user_answer, 0 FROM log0 l JOIN questions q ON q.q_id = l.q_id WHERE q.q_type = 'blank' "
      . "UNION SELECT id, user_answer, 1 FROM log1 l JOIN questions q ON q.q_id = l.q_id WHERE q.q_type = 'blank' "
      . "UNION SELECT id, user_answer, 2 FROM log2 l JOIN questions q ON q.q_id = l.q_id WHERE q.q_type = 'blank' "
      . "UNION SELECT id, user_answer, 3 FROM log3 l JOIN questions q ON q.q_id = l.q_id WHERE q.q_type = 'blank'";
  $blank_answers = $mysqli->prepare($select_sql);
  $blank_answers->execute();
  $blank_answers->store_result();
  $blank_answers->bind_result($id, $answer, $log);
  ...
  while ($blank_answers->fetch()) {
    ...
    $update_sql = "UPDATE log$log SET user_answer = ? WHERE id = ?";
