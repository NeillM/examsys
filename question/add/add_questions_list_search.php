...
<?php
  ...
  if (isset($_GET['search']) or isset($_GET['sortby'])) {
    ...
    if ($_GET['owner'] == '') {
      ...
      $result = $mysqli->prepare("SELECT DISTINCT questions.q_id, q_type, leadin, DATE_FORMAT(last_edited,' {$configObject->get('cfg_short_date')}') AS display_date, locked, status, name FROM (questions_modules, questions, question_statuses) LEFT JOIN options ON questions.q_id=options.o_id WHERE questions.status = question_statuses.id AND questions.q_id = questions_modules.q_id AND (idMod IN ($teamlist) OR questions.ownerID=?) AND (leadin_plain LIKE ? OR theme LIKE ? OR scenario_plain LIKE ? OR notes LIKE ? OR option_text LIKE ?) AND q_type LIKE ? AND deleted IS NULL ORDER BY $sortby $ordering, questions.q_id");
