...
<?php
  ...
  if (stripos('External Examiner', $tmp_roles) !== false) {      // Get the papers the External is down to review.
  ...
  } elseif ($userObject->has_role(array('Admin','SysAdmin')) or $userObject->get_user_ID() == $_GET['userID']) {
    // Only allow Admin/SysAdmin or current user to view this information
    $query_sql = "(SELECT crypt_name, paper_title, 0 AS paper_type, q_paper, DATE_FORMAT(log_metadata.started,'%Y%m%d%H%i%s') AS started, DATE_FORMAT(log_metadata.started,'$configObject->get('cfg_long_date_time')') AS display_started, duration, screen, ipaddress FROM properties, log0, log_metadata WHERE log0.q_paper=log_metadata.paperID AND log0.started=log_metadata.started AND log0.userID=log_metadata.userID AND properties.property_id=log0.q_paper AND log0.userID=?)";
    $query_sql .= " UNION ALL (SELECT crypt_name, paper_title, 1 AS paper_type, q_paper, DATE_FORMAT(log_metadata.started,'%Y%m%d%H%i%s') AS started, DATE_FORMAT(log_metadata.started,'$configObject->get('cfg_long_date_time')') AS display_started, duration, screen, ipaddress FROM properties, log1, log_metadata WHERE log1.q_paper=log_metadata.paperID AND log1.started=log_metadata.started AND log1.userID=log_metadata.userID AND properties.property_id=log1.q_paper AND log1.userID=?)";
    $query_sql .= " UNION ALL (SELECT crypt_name, paper_title, 2 AS paper_type, q_paper, DATE_FORMAT(log_metadata.started,'%Y%m%d%H%i%s') AS started, DATE_FORMAT(log_metadata.started,'$configObject->get('cfg_long_date_time')') AS display_started, duration, screen, ipaddress FROM properties, log2, log_metadata WHERE log2.q_paper=log_metadata.paperID AND log2.started=log_metadata.started AND log2.userID=log_metadata.userID AND properties.property_id=log2.q_paper AND log2.userID=?)";
    $query_sql .= " UNION ALL (SELECT crypt_name, paper_title, 3 AS paper_type, q_paper, DATE_FORMAT(log_metadata.started,'%Y%m%d%H%i%s') AS started, DATE_FORMAT(log_metadata.started,'$configObject->get('cfg_long_date_time')') AS display_started, duration, screen, ipaddress FROM properties, log3, log_metadata WHERE log3.q_paper=log_metadata.paperID AND log3.started=log_metadata.started AND log3.userID=log_metadata.userID AND properties.property_id=log3.q_paper AND log3.userID=?)";
    ...
    $query_sql .= " UNION ALL (SELECT crypt_name, paper_title, 5 AS paper_type, q_paper, DATE_FORMAT(started,'%Y%m%d%H%i%s') AS started, DATE_FORMAT(started,'$configObject->get('cfg_long_date_time')') AS display_started, NULL AS duration, NULL AS screen, NULL AS ipaddress FROM properties, log5 WHERE properties.property_id=log5.q_paper AND log5.userID=?)";
      ...
      for ($i=0; $i<$results_no; $i++) {
        ...
        switch ($paper[$i]['paper_type']) {
          ...
          case 5:
            ...
            break;
          case 6:
            echo "<tr style=\"height:17px\"><td style=\"text-align:right\"><img src=\"../artwork/peer_review_16.gif\" width=\"16\" height=\"16\" alt=\"\" /></td><td>&nbsp;" . $paper[$i]['q_paper'] . "</td><td>" . $string['peerreview'] . "</td><td>" . $paper[$i]['display_started'] . "</td><td style=\"color:#808080\">" . $string['na'] . "</td><td style=\"color:#808080\">" . $string['na'] . "</td></tr>\n";
            break;
        }
      }
      if ($results_no == 0) {
...
<?php
  ...
  for ($i=0; $i<$row_no; $i++) {
    if ($user_modules[$i]['calendar_year'] != $old_year) {
      ...
      if ($user_modules[$i]['calendar_year'] == $most_recent_year or $user_modules[$i]['calendar_year'] == date_utils::get_current_academic_year() ) {
