...
function list_modules($mod, $id, $student_mod, $string, $session)
{
    ...
    foreach ($mod as $idMod => $mod_info) {
        ...
        if ($old_letter != mb_strtoupper(mb_substr((string) $moduleid, 0, 1))) {
            echo '<table border="0" style="padding-bottom:5px; width:100%; color:#1E3287"><tr><td><nobr>&nbsp;' . mb_strtoupper(mb_substr((string) $moduleid, 0, 1)) . "</nobr></td><td style=\"width:98%\"><hr noshade=\"noshade\" style=\"border:0px; height:1px; color:#E5E5E5; background-color:#E5E5E5; width:100%\" /></td></tr></table>\n";
        }
        ...
        $loop++;
        $old_letter = mb_strtoupper(mb_substr((string) $moduleid, 0, 1));
    }
    echo "</div>\n</div>\n";
}
