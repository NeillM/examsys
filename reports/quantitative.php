...
<head>
  ...
  <title>ExamSys: <?php echo $string['quantitativereport']; ?></title>
...
if ($module != '' and $module != 0) {
    echo '<img src="../artwork/breadcrumb_arrow.png" class="breadcrumb_arrow" alt="-" /><a href="../module/index.php?module=' . $module . '">' . $module_code . '</a>';
}
...
while ($result->fetch()) {
    ...
    if ($old_q_id != $q_id and $old_q_id > 0) {   // New question.
        ...
        if ($display_respondents == 1 and $old_q_type != 'info') { // Calculate how many candidates.
            ...
            if (isset($log_array[$old_screen][$old_q_id][1]['y'])) {
                $respondents += $log_array[$old_screen][$old_q_id][1]['y'];
            }
            if (isset($log_array[$old_screen][$old_q_id][1]['n'])) {
                $respondents += $log_array[$old_screen][$old_q_id][1]['n'];
            }
