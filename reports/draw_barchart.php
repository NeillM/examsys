...
for ($label = 0; $label <= $total_possible_mark; $label++) {
    $line_x = floor($g_x1 + $gap1 * $label);
    ImageLine($Image, $line_x, $g_y1, $line_x, $g_y1 - 2, $dkgrey);
}
...
//bars
$gap3 = 3;
// The value is used as an integer co-ordinate we want to avoid implicit conversions.
$gap4 = floor(($g_y2 - $g_y1 + $gap3) / 2);

//student mark bar
$student_mark = round($student_mark, 1);
$student_x = floor($g_x1 + $gap1 * $student_mark);
ImageFilledRectangle($Image, $g_x1, $g_y1 + $gap3, $student_x, $g_y1 + $gap4 - $gap3, $amber);
ImageRectangle($Image, $g_x1, $g_y1 + $gap3, $student_x, $g_y1 + $gap4 - $gap3, $black);
if (mb_strlen($student_mark) > 2) {
    imagettftext($Image, 10, 0, $student_x - 20, $g_y1 + 15, $color, $bold_font, $student_mark);
} elseif ($student_mark > 0) {
    imagettftext($Image, 10, 0, $student_x - 10, $g_y1 + 15, $color, $bold_font, $student_mark);
}

  //median mark bar
$median = round($median, 1);
$mark_x = floor($g_x1 + $gap1 * $median);
ImageFilledRectangle($Image, $g_x1, $g_y1 + $gap4, $mark_x, $g_y2 - $gap3, $color);
ImageRectangle($Image, $g_x1, $g_y1 + $gap4, $mark_x, $g_y2 - $gap3, $black);

if (mb_strlen($median) > 2) {
    imagettftext($Image, 10, 0, $mark_x - 19, $g_y1 + 37, $black, $font, $median);
} elseif ($median > 0) {
    imagettftext($Image, 10, 0, $mark_x - 9, $g_y1 + 37, $black, $font, $median);
}
