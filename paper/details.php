...
<?php
  ...
  $nooptions = array('keyword_based', 'random', 'placeholder', 'textbox');
  for ($x=1; $x<=$row_no; $x++) {
    $status = $status_array[$temp_array[$x]['status']];
    if ($temp_array[$x]['options'] == 0 and isset($temp_array[$x]['o_media']) and count($temp_array[$x]['o_media']) == 0 and !in_array($temp_array[$x]['q_type'], $nooptions)) {
