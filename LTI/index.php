...
if (!$lti->isInstructor()) {
  ...
  if ($returned === false) {
    ...
    foreach ($data as $v) {
      ...
      if (!UserUtils::is_user_on_module($userObject->GetUserID(), $v[1], $session, $mysqli) and $returned_check !== false and $lti_i::allow_module_self_reg($v)) {
        list($fullname, $school, $active, $selfenroll) = $returned_check;
        if ($active == 1 and $selfenroll == 1 and !UserUtils::is_user_on_module($userObject->GetUserID(), $v[1], $session, $mysqli)) {
          // Insert new module enrollment
          UserUtils::add_student_to_module($userObject->GetUserID(), $v[1], 1, $session, $mysqli);
        }
      }
    }
    // do 'something' here
  ...
  if ($returned !== false) {
    ...
    foreach ($data as $v) {
      if (!UserUtils::staff_on_team($v[1], $mysqli) and $lti_i::allow_staff_module_register($v)) {
        UserUtils::add_staff_to_team($userObject->GetUserID(), $v[1], $mysqli);
