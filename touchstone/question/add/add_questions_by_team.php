...
<?php
  ...
  <?php
  echo "<tr><td colspan=\"5\" style=\"background-color:#F1F5FB; font-size:160%; font-weight:bold\">&nbsp;by Team - " .  $_GET['team'] . "</td></tr>\n";
  ...
  $stmt = $mysqli->prepare("SELECT q_id, q_type, leadin, q_media, q_media_width, q_media_height, DATE_FORMAT(last_edited,'%d/%m/%y') AS display_date, locked FROM questions WHERE q_group=? AND deleted IS NULL ORDER BY $order $direction");
  ...
  if ($stmt->num_rows > 0) {
    while ($stmt->fetch()) {
      ...
      echo "</td><td><input onclick=\"parent.top.controls.checkStatus(this)\" type=\"checkbox\" name=\"$q_id\" value=\"$q_id\" /></td><td style=\"padding-left:8px\" onclick=\"Qpreview($q_id)\">$tmp_leadin</td><td>&nbsp;" . fullQuestionType($q_type) . "</td><td>&nbsp;$display_date</td></tr>\n";
    }
    $stmt->close();
  } else {
