...
<?php
  ...
  <?php
  echo "<tr><td colspan=\"4\" style=\"background-color:#F1F5FB; font-size:160%; font-weight:bold\">&nbsp;My Unused Questions</td></tr>\n";
  ...
  $query_string = "SELECT question, q_id, q_type, leadin, q_media, q_media_width, q_media_height, DATE_FORMAT(last_edited,'%d/%m/%y') AS display_date FROM papers RIGHT JOIN questions ON papers.question=questions.q_id WHERE questions.ownerID=$userID AND status != 'retired' AND deleted IS NULL ORDER BY $order $direction";
  ...
  while ($row = $question_data->fetch_assoc()) {
    if ($row['question'] == NULL) {
      ...
      echo "<tr><td><input onclick=\"parent.top.controls.checkStatus(this)\" type=\"checkbox\" name=\"" . $row['q_id'] . "\" id=\"" . $row['q_id'] . "\" value=\"" . $row['q_id'] . "\" /></td><td style=\"padding-left:8px\" onclick=\"Qpreview(" . $row['q_id'] . ")\">$tmp_leadin</td><td>&nbsp;" . fullQuestionType($row['q_type']) . "</td><td>&nbsp;" . $row['display_date'] . "</td></tr>\n";
    }
  }
  $question_data->close();
  $mysqli->close();
