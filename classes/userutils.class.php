...
Class UserUtils {
  ...
  */
   ...
   * @throws Exception
   */
  static function update_student_id($db, $studentid, $userid) {
    $result = $db->prepare("UPDATE sid SET student_id = ? WHERE userID = ?");
    if ($db->error) {
      try {
  ...
  */
  ...
  static function update_user_by_studentid($studentid, $username, $title, $first_names, $surname, $email, $course, $gender, $year, $role, $db, $initials) {
    ...
    self::update_user($userID, $username, $title, $email, $surname, $first_names, $course, $gender, $year, $role, $db, $initials);
  }
   ...
   */
  static function update_user($userID, $username, $title, $email, $surname, $first_names, $course, $gender, $year, $roles, $db, $initials) {
    $result = $db->prepare("UPDATE users SET roles = ?, title = ?, initials = ?, surname = ?, grade = ?, yearofstudy = ?, username = ?, email = ?, first_names = ?, gender = ? WHERE id = ?");
    $result->bind_param('sssssissssi', $roles, $title, $initials, $surname, $course, $year, $username, $email, $first_names, $gender, $userID);
    $result->execute();
    $result->close();
  }
  ...
  /*
   * Remove a member of staff from a team by modulecode.
   *
   * @param integer $tmp_userID UserID of the member of staff
   * @param string $module_code the name of the team (module)
   * @param object $db mysqli database connection
   *
   */
  public static function remove_staff_from_module_by_modulecode($tmp_userID, $module_code, $db) {

    if (!self::has_user_role($tmp_userID, 'Staff', $db)) {
      return;
    }
    $idMod = module_utils::get_idMod($module_code, $db);
    if ($idMod !== false) {
      self::remove_staff_from_module($tmp_userID, $idMod, $db);
    }
  }
