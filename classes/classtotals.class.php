...
class ClassTotals
{
    ...
    public function generate_stats()
    {
        ...
        for ($i = 0; $i < $this->user_no; $i++) {
            ...
            if (isset($this->user_results[$i]['mark']) and $this->user_results[$i]['visible']) {
                ...
                if ($tmp_percent < $this->pass_mark) {
                    $this->stats['failures']++;
                }
                if ($tmp_percent < $this->stats['min_percent']) {
                    $this->stats['min_percent'] = $tmp_percent;
                }
                if ($tmp_percent > $this->stats['max_percent']) {
                    $this->stats['max_percent'] = $tmp_percent;
                }

                if ($tmp_percent >= $this->pass_mark and $tmp_percent < $this->distinction_mark) {
                    $this->stats['passes']++;
                }
                if ($tmp_percent >= $this->distinction_mark) {
                    $this->stats['honours']++;
                }
                if ($tmp_mark < $this->stats['min_mark']) {
                    $this->stats['min_mark'] = $tmp_mark;
                }
                if ($tmp_mark > $this->stats['max_mark']) {
                    $this->stats['max_mark'] = $tmp_mark;
                }
        ...
        if ($this->stats['min_mark'] == 9999) {
            $this->stats['min_mark'] = 0;    // Reset back to zero if still on default.
        }
    ...
    public function sort_results()
    {
        ...
        if ($tmp_sort == 'adj_percent') {
            $tmp_sort = 'mark';
        }
        if ($tmp_sort == 'classification') {
            $tmp_sort = 'mark';
        }
    ...
    public function check_late_submission_warnings()
    {
      ...
      <table border="0" cellpadding="0" cellspacing="0" style="width:100%">
        <tr>
          ...
          <td class="redwarn"><?php echo $this->string['latesubmissionsmsg'] . ' (<a style="color:black" href="#" class="latelink" onclick="return false;">' . $this->string['moredetails'] . '</a>)'; ?></td>
        </tr>
      </table>
            <?php
        }
