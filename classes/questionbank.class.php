...
class QuestionBank {
  ...
  private function load_stats($type) {
    ...
    switch ($type) {
      case 'all':
      case 'type':
        $sql = 'SELECT COUNT(questions.q_id), q_type FROM questions, questions_modules WHERE questions.q_id = questions_modules.q_id AND idMod = ? AND ownerID = ? AND deleted IS NULL GROUP BY q_type';
        break;
      case 'status':
        $sql = 'SELECT COUNT(questions.q_id), name FROM questions, questions_modules, question_statuses WHERE questions.status = question_statuses.id AND questions.q_id = questions_modules.q_id AND idMod = ? AND ownerID = ?  AND deleted IS NULL GROUP BY status';
        break;
      case 'bloom':
        $sql = 'SELECT COUNT(questions.q_id), bloom FROM questions, questions_modules WHERE questions.q_id = questions_modules.q_id AND idMod = ? AND ownerID = ?  AND deleted IS NULL GROUP BY bloom';
        break;
      case 'keyword':
        $sql = 'SELECT COUNT(questions.q_id), keywordID FROM questions, questions_modules, keywords_question, keywords_user WHERE keywords_question.keywordID = keywords_user.id AND questions.q_id = questions_modules.q_id AND idMod = ? AND ownerID = ?  AND questions.q_id = keywords_question.q_id AND deleted IS NULL GROUP BY keywordID';
        break;
      case 'objective':
