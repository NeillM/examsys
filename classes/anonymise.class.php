...
class Anonymise
{
    ...
    {
        // Update the user table (Do NOT update SysAdmin, SysCron accounts or the temp accounts: user1, user2, etc).
        $sql = "
        SELECT
            u.id, title, gender
        FROM
    ...
    private function get_email($first_names, $surname)
    {
        ...
        $email = mb_strtolower($parts[0]) . '.' . mb_strtolower(str_replace(' ', '', $surname)) . '@example.com';
